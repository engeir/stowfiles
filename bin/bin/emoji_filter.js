...
function svg_to_pdf(src) {
  ...
  const cmd_line =
    `"${inkscape_path}" --export-type=pdf "${full_target}" "${full_src}"`;
  if (!fs.existsSync(full_target)) shell.exec(cmd_line);
  return full_target;
}
...
async function get_emoji(icon, source) {
  ...
  if (source == "noto-emoji") {
    src =
      `https://raw.githubusercontent.com/googlefonts/noto-emoji/master/svg/emoji_u${icon}.svg`;
    dirname = "noto-emoji";
...
async function replace_emojis(text, format, emoji_source, context) {
  ...
  var split = text_w_img.split(
    /\<img class="emoji" draggable="false" alt="([^"]+)" src="([^"]+)"\/>/g
  );
  if (split.length == 1) return pandoc.Str(split[0]);
  ...
  for (var it = 0; it < split.length; it += 3) {
    ...
    if (it + 2 < split.length && split[it + 2] !== null) {
      ...
      if (format == "latex" && (context == "Verbatim" || context == "texttt")) {
        ...
        img_emoji = pandoc.Image([id, classes, attrs], caption_list, [
          src,
          "fig:",
        ]);
      }
...
async function codeblock_to_verbatim(code_text, format, emoji_source) {
  ...
  return [
    pandoc.RawBlock(
      ...
      "\\begin{"
        + context
        + "}[commandchars=\\\\\\{\\}, mathescape, gobble=\\autogobble]"
    ),
...
async function visit(obj, format, meta) {
  ...
  if (meta.__debug && JSON.parse(meta.__debug.c)) wait_debugger();

  const emoji_source = meta.emoji
    ? meta.emoji.c
    : process.env["emoji_source"] || "noto-emoji";
  if (type === "Str") {
