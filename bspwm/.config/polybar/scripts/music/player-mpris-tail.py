...
class Player:
    ...
    def _parseMetadata(self):
        if self._metadata is not None:
            ...
            _length_int = _length if type(_length) is int else int(float(_length))  # type: ignore[arg-type]
            ...
            _year = _date[0:4] if len(_date) else ""  # type: ignore[index, arg-type]
            ...
            self.metadata["track"] = _track  # type: ignore[assignment]
            self.metadata["genre"] = re.sub(
                SAFE_TAG_REGEX, """\1\1""", _metadataGetFirstItem(_genre)
            )
            self.metadata["disc"] = _disc  # type: ignore[assignment]
            self.metadata["date"] = re.sub(SAFE_TAG_REGEX, """\1\1""", _date)  # type: ignore[arg-type]
            self.metadata["year"] = re.sub(SAFE_TAG_REGEX, """\1\1""", _year)  # type: ignore[arg-type]
            self.metadata["url"] = _url  # type: ignore[assignment]
            self.metadata["filename"] = os.path.basename(_url)  # type: ignore[arg-type]
...
parser.add_argument("--icon-stopped", default="󰏤")
