...
local function mouse_click(env)
  if env.BUTTON == "right" then
    os.execute(
      "yabai -m space --destroy " .. env.SID .. " && sketchybar --trigger space_change"
    )
  else
...
local function space_selection(env)
  ...
  if apps ~= "" then
    local file = assert(
      io.popen(
        "~/stowfiles/sketchybar/.config/sketchybar/plugins/icon_map_fn.sh " .. apps[1]
      )
    )
...
space_creator:subscribe(
  "mouse.clicked",
  function(_) os.execute("yabai -m space --create && sketchybar --trigger space_change") end
)
